...
export class AddPatientComponent implements OnInit {
  ...
  toROCDate(gregorianDateStr: string | null | undefined): string {
    if (!gregorianDateStr) return '';
    const d = new Date(gregorianDateStr);
    if (isNaN(d.getTime())) return '';
  
    const rocYear = d.getFullYear() - 1911;          // no zero-padding: 30, not 030
  ...
  /**
   * Computes full years of age from a Gregorian ISO date string to today.
   * Correctly handles whether the birthday has already passed this year.
   * Returns '' when the input is null / empty / unparseable.
   */
  ...
  downloadTemplate(): void {
    this.patientService.getLTCPatients().subscribe({
      next: (patients: LTCPatient[]) => {
        ...
        // Sort by room number then bed number (numeric)
        const sorted = [...patients].sort((a, b) => {
          const roomDiff = Number(a.room_number) - Number(b.room_number);
          if (roomDiff !== 0) return roomDiff;
          return Number(a.bed_number) - Number(b.bed_number);
        });
        ...
        // Map to report columns matching the source layout
        const reportData = sorted.map(p => ({
          '住民ID(床號)': `${p.room_number}-${p.bed_number}`,
          '姓名':         p.name            ?? '',
          '生日':         this.toROCDate(p.birthdate),   // Gregorian → ROC, no zero-padded year
          '年齡':         this.calcAge(p.birthdate),
          '性別':         SEX_MAP[p.sex] ?? p.sex ?? '',
          '身高':         p.height_cm ?? '',
          '體重':         p.weight_kg ?? '',
          '活動量':       ACTIVITY_MAP[p.activity_level] ?? p.activity_level ?? '',
        }));
        ...
        import('xlsx').then(XLSX => {
          ...
          XLSX.utils.book_append_sheet(workbook, worksheet, '住民資料');
          ...
          import('file-saver').then(fileSaver => {
            fileSaver.saveAs(blob, `住民資料-${todayFormatted}.xlsx`);
          });
        });
      },
 
      error: (err) => {
        console.error('Error fetching LTC patients:', err);
      },
    });
  }
  ...
  uploadExcelFile(): void {
    ...
    this.patientService.addPatientsUsingExcel(this.selectedFile).subscribe({
      next: (response) => {
        ...
        successMessage += '住民資料已更新\n';

        if (response.created_count) {
          successMessage += `新增了 ${response.created_count} 位住民\n`;
        }
        if (response.updated_count) {
          successMessage += `更新了 ${response.updated_count} 位住民\n`;
        }
        if (response.skipped_count) {
          successMessage += `跳過了 ${response.skipped_count} 位住民\n`;
        }

        const totalProcessed = (response.created_count || 0) + (response.updated_count || 0);
        successMessage += `\n總共處理了 ${totalProcessed} 筆記錄`;

        alert(successMessage);

        setTimeout(() => {
          console.log('Redirecting to patient-info...');
